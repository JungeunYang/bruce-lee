$(function () {
  // 기본 동작 테스트
  // gsap.from('대상', {동작옵션:css의 속성값 or gsap가 제공하는 속성값})
  // 1. css 속성/값 적용
  // gsap.from('.logo', { transform: 'trnaslateY(-100px)' });
  // 2. gsap 속성/값 적용 & delay 조정으로 애니메이션 적용
  // gsap.from('.logo', { y: -100, autoAlpha: 0, duaration: 0.5 });
  ...
  TL.from('nav > a', { y: -100, autoAlpha: 0, duration: 1, stagger: 0.2 });
  // stagger: a가 개별적으로 떨어짐
  TL.from('.menu', { y: -100, autoAlpha: 0 });
  TL.from('.logo', { y: -100, autoAlpha: 0, duaration: 0.5, ease: 'bounce.out' });

  TL.from('.foot-box', { width: 0, autoAlpha: 0 });
  TL.to('.logo', { rotation: 360, ease: 'none' /* repeat: -1, duration: 2 */ });
  // repeat: -1을 주면 infinite효과 무한반복
  TL.from('.sns-link li', { autoAlpha: 0, x: -50, stagger: 0.1 });
  /* autoAlpha:0 = opactiy:0 */
  TL.from('.copyright', { autoAlpha: 0 }, '-=.3');
  // {}이후에, 숫자 넣으면 전체 타임라인에서의 몇초에 해당할때 효과 보여줘라 라는 것
  // '-=.3'바로 앞쪽에 있는 애니메이션 요소인 '.sns-link li' 보다 0.3초 전에
  // '<': 바로 앞쪽에 있는 애니메이션 요소와 같은 시점에 애니메이션 실행
  ...
  TL.from(
    '.title h2 span',
    {
      ...
      /* onComlete 메서드 함수: span에 관련된 애니메이션이 끝난 다음에 Splitting이 동작되게 */
    },
    '-=.8'
  );
